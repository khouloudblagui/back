...
export class AppointmentsComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  filterToggle = false;
  displayedColumns = ['select', 'img', 'name', 'dateTime', 'email', 'mobile', 'disease', 'actions'];
  ...
  public loadData() {
    const doctorId = 1; // Remplace par l'ID dynamique du médecin si nécessaire
    this.exampleDatabase = new AppointmentsService(this.httpClient);
    this.dataSource = new ExampleDataSource(this.exampleDatabase, this.paginator, this.sort, doctorId);
  ...
  exportExcel() {
    const exportData: Partial<TableElement>[] = this.dataSource.filteredData.map((x) => ({
      'Patient Name': x.name,
      Email: x.email,
      'Date & Time': formatDate(new Date(x.dateTime), 'yyyy-MM-dd', 'en') || '',
      Mobile: x.mobile,
      Disease: x.disease,
    }));
...
export class ExampleDataSource extends DataSource<Appointments> {
  ...
  connect(): Observable<Appointments[]> {
    ...
    this.exampleDatabase.getAppointmentsByDoctor(this.doctorId);
      ...
      map(() => {
        this.filteredData = this.exampleDatabase.data
          ...
          .filter((appointments: Appointments) => {
            const searchStr = (appointments.name + appointments.dateTime + appointments.email + appointments.mobile).toLowerCase();
